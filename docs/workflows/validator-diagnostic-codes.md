...
| `defconst-value-out-of-range` | lint | warning | active | explain | A numeric `defconst` value is outside the observed signed 32-bit range of -2147483648 to 2147483647. AoE2 DE validated one-step overflow values as loading but clamping to the nearest signed-32 boundary. |
...
| `unvalidated-train-target` | lint | warning | active | explain | A symbol is known in local validation notes, but is not validated for the current direct or dynamic train-target context. |
...
- Corpus profile: active
- Cursor action: explain
- Meaning: A numeric `defconst` value is outside the observed signed 32-bit range of -2147483648 to 2147483647. AoE2 DE validated one-step overflow values as loading but clamping to the nearest signed-32 boundary.
- References:
  - [Internal validation notes](internal-validation-notes.md#defconst-numeric-range)
...
<a id="diagnostic-duplicate-load-target"></a>
...
<a id="diagnostic-site-specific-train-alias-requires-defconst"></a>
...
<a id="diagnostic-unvalidated-train-target"></a>
...
- Corpus profile: active
- Cursor action: explain
- Meaning: A symbol is known in local validation notes, but is not validated for the current direct or dynamic train-target context.
